module.exports = {
    ...
    */
    ...
    'list': function(template, _, engine) {
        ...
        if (!match || match[3]) throw new Error('list tag error');
        var count = 1,
            start = match.index,
            startLabel = match[0],
            dataLabel = match[1],
        ...
        do {
            ...
            if (!match) throw new Error('list tag error');
            if (match[3]) {
                count--;
            } else {
                count++;
        ...
        if (matchRepeat.length === 2) { //解析 <#list 1..item.count as idx>
            ...
            jsString.push('var arr = ', engine.exp(dataLabel), ';');
        }
    ...
    'if': function(template, _, engine) {
        ...
        if (!match || !match[1]) throw new Error('if tag error');
        var count = 1,
            start = match.index,
        ...
        do {
            ...
            if (!match) throw new Error('if tag error');
            if (match[1]) {
                count++;
            } else if (match[2]) {
                if (count === 1) {
